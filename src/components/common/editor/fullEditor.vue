<template>
<!--
<full-editor init-text="- 경기 관람을 원하시는 일자의 최소 7일 전까지 신청 주셔야 가능합니다. " // 에디터에 초기에 입력할 데이터 전달.
             :width="930" // editor width
             :height="458" // editor height
             :fetch="onFetch()" // 이미지 데이터 전송을 위한 api 함수 전달.
...
export default class FullEditor extends Vue{
  ...
  public editorInit: any = {
    ...
    toolbar: 'undo redo | bold italic underline strikethrough | fontsizeselect formatselect| forecolor backcolor removeformat | align | outdent indent | numlist bullist | insertfile | image media link table', //preview fontselect, charmap, emoticons, codesample, alignleft, aligncenter, alignright, alignjustify
  ...
  public created() {
    ...
    this.serverURL=process.env.VUE_APP_API_BASE_URL;
  ...
  public setupEditor(editor: any) {
    ...
    // console.log( 'editorTextBindingModel=', this.editorTextBindingModel );
    ...
    */
    ...
    if (this.fetch) {
      this.fetch( formData )
          .then( (data: any)=>{
            // console.log( data );
            const imgURL=`${ this.serverURL }${ AttachFile.BASE_URL }/${ data.id }`;
  ...
  public getFilePicker( callback: any, value: any, meta:any ) {
    ...
    if (meta.filetype === 'image') {
      ...
      const uploadBtn= ( document.getElementById(`${this.uploadId}`) as HTMLElement);
...
<style lang="scss">
/*.tox .tox-tbtn{width:22px !important;height:22px !important;font-size: 12px !important;}
.tox .tox-tbtn--select{width:auto !important;}*/
